...
decoder_NEC_handler decoder_inst = &decoder_storage;
 ...
 * `IR_DATA_0` by a margin of 0, then bit shift corresponding value. Bit shift works by creating value
 ...
 */
static void _populate_metdata(uint8_t *metadata, uint32_t *buffer, int start_idx)
{
...
{
    if( _within_margin(buffer[0], STEUP_START, MARGIN_OF_ERROR) ){
        if(_within_margin(buffer[1], REPEAT_TRANSMISSION, 1)){
            return;
        }
        else if(_within_margin(buffer[2], NEW_TRANSMISSION, MARGIN_OF_ERROR)){
    ...
    printf("low addr\n");
    _populate_metdata( &(decoder_inst->address_low), buffer, 3);    
    printf("\ninverse low addr\n");
    _populate_metdata( &(decoder_inst->inverted_low_address), buffer, 19);    
    printf("\nhigh addr\n");
    _populate_metdata( &(decoder_inst->address_high), buffer, 35);    
    printf("\ninverse high addr\n");
    _populate_metdata( &(decoder_inst->inverted_high_address), buffer, 51);    
    printf("\ncommand 1\n");
    _populate_metdata( &(decoder_inst->command), buffer, 67);    
    printf("\ncommand 2\n");
    _populate_metdata( &(decoder_inst->inverted_command), buffer, 83);    
}
